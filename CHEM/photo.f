...
c-AJ 0826/22 comment the output
c      IF (N.EQ.1) write(90,119)
c 119  FORMAT(/1X,'ENERGY FLUXES IN W/m^2/nm and photons/m^2/s/nm
c     &  (NOT DIURNALLY AVERAGED)')
c      IF (N.eq.1) write(90,118)
c 118  FORMAT(/2X,'L',6X,'WAV',10X,'EFLUX',9X,'GFLUX',
c     & 10X,'S(1)',8x,'PhEFLUX',8x,'PhGFLUX')
...
C ***** SET PHOTOLYSIS RATES TO ZERO *****
      ...
      PNO2(I) = 0.
      PHNO4(I) = 0.
c in may 7 2019, JL rip off cl in photo
...
C       ADDITIONAL MAJOR ABSORBERS. *****
      HA = RMG*T(NZ)
      HAD = HA*DEN(NZ)
C-AJ ZERO OUT FOR GAUSS COMPARISON
      TTOT(NZ) = DEN(NZ)*HA
      TO2(NZ) = O2(NZ)*HAD
      TO3(NZ) = O3(NZ)*HAD
      TCO2(NZ) = CO2(NZ)*HAD 
...
c      print *, 'TO2 = ', TO2 disagree 
c      print *, 'EFAC =', EFAC off by factor of 100 
c      print *, 'O2(I) = ', O2(I) same 

c       print *, 'DZ = ', DZ  same 
c        print *, 'HA = ', HA not the same

c       print *, 'RMG =', RMG off a bit 
c       print *, ' T(I) = ',T(I)  
...
c      DO 17 K=1,KMAX
c      DO 17 I=1,NZ
c  17  CL(I) = CL(I) + B(L,K)*TO2L(I)**(K-1)
C
      DO 18 I=1,NZ
      C = 10.**CL(I)
      SD = SIG0(I,L) * AM**(-C)
...
c-JK      IF(L.LE.17) GO TO 20
C-JK  This will skip the two-steam in the SR wavelength region
C
      CALL TWOSTR(WAV(L),SIGR,U0,SO3,SO2,SCO2(L),SH2O(L),SSO2T,
     2 SH2S(L))
c************** tEMPORARY! Zeroing out CO2 and H2O as major absorbers ***********
c SO3 is a vector, if we want to zero it out as the absorber, we need to let SO3(I)=0. for all layers (NZ)
C SCO2,SH2O are both scalars.
c      CALL TWOSTR(WAV(L),SIGR,U0,SO3,SO2,0.,0.,SSO2T,
c     2 SH2S(L))
      M=1
      GO TO 21
C
...
c      print *, 'DTAU =', DTAU all prints zero

c      print *, 'flx = ', FLX . Note: they are the same
c      print *, 'SO3(I) = ', SO3(I) same 
c       print *, 'RO3 = ', RO3 : they are the same 
c       print *, ' S(1) =', s(1)
c       print *, 'AM =',AM same 
c      print *, ' sigo2 = ', sigo2 
c       print *, 'TO2(I) = ', TO2(I) off by a factor of 2 
...
c in may 7 2019, jl rip off all cl 
C-AJ 08/30/2022 ADD CL BACK
...
c      print *, ' I =', I 
c      print *, 'flx = ', FLX may 14, they are the same
c       print *, 'SO3(I) = ', SO3(I) , may 14, they are the same 
c       print *, 'RO3 = ', RO3 : they are the same 
c       print *, ' S(I) =', S(I) they are the same  
C-AP    
...
C-AJ 03/14/22 SKIP THE SCATTERING
C      IF(L .LE. 108) GO TO 51
...
c      print *, ' SIGO2 =', SIGO2
c       print *, ' TO2 = ', TO2 : they are identical to the older one 
c      print *, 'SO3 = ', SO3
c      print *, 'TO3 = ', TO3 
c      print *, 'SH2O = ', SH2O 
c      print *, 'TH2O = ', TH2O 
...
c      if(L.eq.55)print *,'In PHOTO, AT, BT,CT,WAV,ALMO,RO3,T'
      ...
      RO3 = AMAX1(RO3,0.)
      RO3 = AMIN1(RO3,0.9)
c      if(L.eq. 55) Print 1500,AT(I), BT(I),CT(I),WAV(L),ALM0(I), 
c     2 RO3,T(I)
c 1500 format(1x,1p7e10.3)
c      IF(I.eq.1) Print *,'    RO3 =',RO3
      PO3D(I) = PO3D(I) + FLX*SO3(I)*RO3*S(I)
      PO3(I) = PO3(I) + FLX*SO3(I)*(1.-RO3)*S(I)


c       print *, ' BT(I) = ', BT(I): wrong, constant at -2.2 
c      print *, 'BL = ', BL not the same 
...
C-AJ 08/30/2022 ADD CL BACK
      ...
      pcl2o2(i) = pcl2o2(i) + flx*scl2o2(l)*s(i)
C-APerror #6351: The number of subscripts is incorrect. 
      PSO21(I) = PSO21(I) + FLX*SSO21(L)*S(I)
...
c-AJ 0826/22 comment the output
c      write(90,120) L1,XL(L1),ESFX(L1),GSFX(L1),
c     & S1(L1),PhESFX(L1),PhGSFX(L1)
      enddo
...
c      print *, 'gflux =', gflux 
c      print *, 'SS =', SS  off by a factor of 100 
 
c Converting FLUX from photons/cm^2/s to photons/m^2/s/nm
      PhEFLUX(L) = 1.e4*FLUX(L)/(1.e-1*DELWAV)
      PhGFLUX(L) = PhEFLUX(L)*SS(L)
c-AJ 0826/22 comment the output
c      write(90,120) L,WAV(L),EFLUX(L),GFLUX(L),
c     & SS(L),PhEFLUX(L),PhGFLUX(L)
      enddo
