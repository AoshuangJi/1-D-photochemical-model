...
C      PARAMETER(NZ1=NZ-1, NQI=31, NSLS=38)
...
C
C      DATA LH2CQ,LQ,LH2Q,LQH,LHOQ,LH2OQ,LO2Q,LOQO,LCQ,LCH3OQH,
...
C     LONG-LIVED SPECIES
      ...
      ISPECI(19) = 6HHOQNO2 !HO2NO2(A)
      ISPECI(20) = 6HHO2NOQ !HO2NO2(B)
      ...
      ISPECI(28) = 3HHSQ    !HSO 
      ISPECI(29) = 3HCOQ    !CO2
! AJ 01/23/2020 ADD HQO AND HQONO2
C      ISPECI(30) = 3HHQO    !HO2(B)
C      ISPECI(31) = 6HHQONO2 !HO2NO2(C)      
...
! AJ 01/23/2020 ADD HQO AND HQONO2
C        SLI(LHQO,J) = SL(LHO2,J)
C        SLI(LHQONO2,J) = SL(LHO2NO2,J)
C************************************************** 
c  jk  Initialize the 'inert' species (i.e., those with no Q's)         
...
!01/23/2020 JL ADD TWO SPECIES 
C        SGFLUXI(LHQO) = 0.0!SGFLUX(LHO2)   
C        SGFLUXI(LHQONO2) = 0.0!SGFLUX(LHO2NO2)   
...
!01/23/2020 JL ADD TWO SPECIES 
C        VDEPI(LHQO) = VDEP(LHO2)
C        VDEPI(LHQONO2) = VDEP(LHO2NO2)
C******UPPER BOUNDARY***********************************
C********UPWARD FLUX************************************
...
C        SMFLUXI(LHQO) = 0.0!SMFLUX(LHO2)
C        SMFLUXI(LHQONO2) = 0.0!SMFLUX(LHO2NO2)
C********Effusion velocity******************************
...
! AJ 01/23/2020 ADD TWO SPECIES
C        VEFFI(LHQO) = VEFF(LHO2)
C        VEFFI(LHQONO2) = VEFF(LHO2NO2)
...
! AJ 01/23/2020 ADD TWO SPECIES
C        LBOUNDI(LHQO) = LBOUND(LHO2)
C        LBOUNDI(LHQONO2) = LBOUND(LHO2NO2)
C********Upper boundary******************************
...
! AJ 01/23/2020 ADD TWO SPECIES
C        MBOUNDI(LHQO) = MBOUND(LHO2)
C        MBOUNDI(LHQONO2) = MBOUND(LHO2NO2)
C   0 = CONSTANT DEPOSITION VELOCITY (VDEP)
C   1 = CONSTANT MIXING RATIO
C   2 = CONSTANT UPWARD FLUX (SGFLUX)

C  01/28/2020 JL INITIATE HQO AND HQONO2 
...
C        USOLI(30,J) =DIZ(5,J)/DEN(J)
C        USOLI(31,J) = DIZ(19,J)/DEN(J) 
c        USOLI(,J) =USOL(5,J)
c        USOLI(31,J) = USOL(19,J) 
...
C AJ 01/28 ADD HQO AND HQONO2
C      print 206,'Alt(km)',(ISPECI(I),I=30,31)
C      DO J=1,3
C      print 203,j-0.5,(DIZ(I,J)/DEN(J),I=30,31)
C      END DO   
C      DO J=4,NZ,2
C      print 203,j-0.5,(DIZ(I,J)/DEN(J),I=30,31)
C      ENDDO
...
C JK Temporary *** Skip this for now!
      EMAX = 0.
c      IS = 0
c      JS = 0
c      go to 126
...
!         END IF  
         ...
         PRINT *, 'Hello World'
        print *, 'SLI(LHOQNO2)=',SLI(LHOQNO2,1)
        print *, 'SL(LHO2NO2)=',SL(LHO2NO2,1)
...
C 189  FORMAT(6X,'HOQNO2',6X,'HO2NOQ',7X,'NO2Q',7X,'N2O4Q',7X,'N2QO4',
C     & 8X,'OQ',11X,'SQ',9X,'SOQ',7X,'H2SO3Q',8X,'HSQ',9X,'COQ')  
C 192  FORMAT(6X,'HO2NO2',6X,'HO2NO2',7X,'NO3', 8X,'N2O5', 8X,'N2O5',
C     & 9X,'O2',11X,'SO',9X,'SO2',8X,'H2SO4'8X,'HSO',9X,'CO2')

C 193  FORMAT(6X,'Q1D',9X,'H3CQ',10X,'HCQ',7X,'SQ21',8X,'SQ23',
C     & 8X,'HSO2Q',9X,'SO2Q')  
C 194  FORMAT(6X,'O1D',9X,'H3CO',10X,'HCO', 7X,'SO21', 8X,'SO23',
C     & 9X,'HSO3',9X,'SO3')

 189  FORMAT(6X,'HOQNO2',6X,'HO2NOQ',7X,'NO2Q',7X,'N2O4Q',7X,'N2QO4',
     & 8X,'OQ',11X,'SQ',9X,'SOQ',7X,'H2SO3Q')  
 192  FORMAT(6X,'HO2NO2',6X,'HO2NO2',7X,'NO3', 8X,'N2O5', 8X,'N2O5',
     & 9X,'O2',11X,'SO',9X,'SO2',8X,'H2SO4')

 193  FORMAT(6X,'Q1D',9X,'H3CQ',10X,'HCQ',7X,'SQ21',8X,'SQ23',
     & 8X,'HSO2Q',9X,'SO2Q',8X,'HSQ',9X,'COQ')  
 194  FORMAT(6X,'O1D',9X,'H3CO',10X,'HCO', 7X,'SO21', 8X,'SO23',
     & 9X,'HSO3',9X,'SO3',8X,'HSO',9X,'CO2')

C 195  FORMAT(6X,'HQO',9X,'HQONO2')  
 183  FORMAT(6X,'HO2',9X,'HO2NO2')

 182  FORMAT(3X,1P9E12.5)
C 183  FORMAT(3X,1P11E12.5)
...
C AJ 01/30
C     PRINT 189
C     PRINT 183, USOLI(LHOQNO2,1),USOLI(LHO2NOQ,1),
C    * USOLI(LNO2Q,1),USOLI(LN2O4Q,1),USOLI(LN2QO4,1),
C    * USOLI(LOQ,1),USOLI(LSQ,1),USOLI(LSOQ,1),
C    * USOLI(LH2SO3Q,1),USOLI(LHSQ,1),USOLI(LCOQ,1)
C     PRINT 183, USOLI(LHOQNO2,10),USOLI(LHO2NOQ,10),
C    * USOLI(LNO2Q,10),USOLI(LN2O4Q,10),USOLI(LN2QO4,10),
C    * USOLI(LOQ,10),USOLI(LSQ,10),USOLI(LSOQ,10),
C    * USOLI(LH2SO3Q,10),USOLI(LHSQ,10),USOLI(LCOQ,10)
C     PRINT 192
C     PRINT 183, USOL(LHO2NO2,1),USOL(LHO2NO2,1),USOL(LNO3,1),
C    * USOL(LN2O5,1),USOL(LN2O5,1),USOL(LO2,1),
C    * USOL(LSO,1),USOL(LSO2,1),USOL(LH2SO4,1)
C    * ,USOL(LHSO,1),USOL(LCO2,1)
C     PRINT 183, USOL(LHO2NO2,10),USOL(LHO2NO2,10),USOL(LNO3,10),
C   * USOL(LN2O5,10),USOL(LN2O5,10),USOL(LO2,10),
C     * USOL(LSO,10),USOL(LSO2,10),USOL(LH2SO4,10)
C     * ,USOL(LHSO,10),USOL(LCO2,10)

      PRINT 189
      PRINT 182, USOLI(LHOQNO2,1),USOLI(LHO2NOQ,1),
     * USOLI(LNO2Q,1),USOLI(LN2O4Q,1),USOLI(LN2QO4,1),
     * USOLI(LOQ,1),USOLI(LSQ,1),USOLI(LSOQ,1),
     * USOLI(LH2SO3Q,1)
      PRINT 182, USOLI(LHOQNO2,10),USOLI(LHO2NOQ,10),
     * USOLI(LNO2Q,10),USOLI(LN2O4Q,10),USOLI(LN2QO4,10),
     * USOLI(LOQ,10),USOLI(LSQ,10),USOLI(LSOQ,10),
     * USOLI(LH2SO3Q,10)
      PRINT*, ''
      PRINT 192
      PRINT 182, USOL(LHO2NO2,1),USOL(LHO2NO2,1),USOL(LNO3,1),
     * USOL(LN2O5,1),USOL(LN2O5,1),USOL(LO2,1),
     * USOL(LSO,1),USOL(LSO2,1),USOL(LH2SO4,1)     
      PRINT 182, USOL(LHO2NO2,10),USOL(LHO2NO2,10),USOL(LNO3,10),
     * USOL(LN2O5,10),USOL(LN2O5,10),USOL(LO2,10),
     * USOL(LSO,10),USOL(LSO2,10),USOL(LH2SO4,10)
      PRINT*, ''     
      PRINT 193
      PRINT 182, USOLI(LQ1D,1),USOLI(LH3CQ,1),USOLI(LHCQ,1),
     * USOLI(LSQ21,1),USOLI(LSQ23,1),USOLI(LHSO2Q,1),USOLI(SO2Q,1),
     * USOLI(HSQ,1),USOLI(LCOQ,1)
      PRINT 182, USOLI(LQ1D,10),USOLI(LH3CQ,10),USOLI(LHCQ,10),
     * USOLI(LSQ21,10),USOLI(LSQ23,10),USOLI(LHSO2Q,10),USOLI(SO2Q,10),
     * USOLI(HSQ,10),USOLI(LCOQ,10)
      PRINT 194
      PRINT 182, USOL(LO1D,1),USOL(LH3CO,1),USOL(LHCO,1),
     * USOL(LSO21,1),USOL(LSO23,1),USOL(LHSO3,1),USOL(SO3,1),
     * USOL(HSO,1),USOL(LCO2,1)
      PRINT 182, USOL(LO1D,10),USOL(LH3CO,10),USOL(LHCO,10),
     * USOL(LSO21,10),USOL(LSO23,10),USOL(LHSO3,10),USOL(SO3,10),
     * USOL(HSO,10),USOL(LCO2,10)
      PRINT*, ''     
C      PRINT 195
C      PRINT 184, USOLI(LHQO,1),USOLI(LHQONO2,1)
C      PRINT 184, USOLI(LHQO,10),USOLI(LHQONO2,10)
      PRINT 183
      PRINT 184, USOL(LHO2,1),USOL(LHO2NO2,1)
      PRINT 184, USOL(LHO2,10),USOL(LHO2NO2,10)
C 
...
!     * SL(LSO3,10),SL(LS3,10),SL(LS4,10)
     ...
     * SL(LNO2,10),SL(LHNO2,10),SL(LHNO3,10)
      PRINT*, ''
      PRINT 189
C JL GET RIDE OF 1/2.FOR HOQNO2
      PRINT 182, DIZ(LHOQNO2,1),DIZ(LHO2NOQ,1),DIZ(LNO2Q,1),
     * DIZ(LN2O4Q,1),DIZ(LN2QO4,1),DIZ(LOQ,1),
     * DIZ(LSQ,1),DIZ(LSOQ,1),DIZ(LH2SO3Q,1)
      PRINT 182, DIZ(LHOQNO2,10),DIZ(LHO2NOQ,10),DIZ(LNO2Q,10)
     * ,DIZ(LN2O4Q,10),DIZ(LN2QO4,10),DIZ(LOQ,10),
     * DIZ(LSQ,10),DIZ(LSOQ,10),DIZ(LH2SO3Q,10)

      PRINT 192
      PRINT 182, SL(LHO2NO2,1),SL(LHO2NO2,1),SL(LNO3,1),
     * SL(LN2O5,1),SL(LN2O5,1),SL(LO2,1),
     * SL(LSO,1),SL(LSO2,1),SL(LH2SO4,1)

      PRINT 182, SL(LHO2NO2,10),SL(LHO2NO2,10),SL(LNO3,10),
...
C      PRINT*, ''
C      PRINT 193
C      PRINT 182, DIZ(LQ1D,1),DIZ(LH3CQ,1),DIZ(LHCQ,1),
C     * DIZ(LSO1Q,1),DIZ(LSO3Q,1),DIZ(LHSO2Q,1),DIZ(LSO2Q,1)
C     * ,DIZ(LHSQ,1),DIZ(LCOQ,1)
C      PRINT 182, DIZ(LQ1D,10),DIZ(LH3CQ,10),DIZ(LHCQ,10),
C     * DIZ(LSO1Q,10),DIZ(LSO3Q,10),DIZ(LHSO2Q,10),DIZ(LSO2Q,10)
C     * ,DIZ(LHSQ,10),DIZ(LCOQ,10)
C      PRINT 194
C      PRINT 182, SL(LO1D,1),SL(LH3CO,1),SL(LHCO,1),
C     * SL(LSO21,1),SL(LSO23,1),SL(LHSO3,1),SL(LSO3,1)
C     * ,SL(LHSO,1),SL(LCO2,1)
C      PRINT 182, SL(LO1D,10),SL(LH3CO,10),SL(LHCO,10),
C     * SL(LSO21,10),SL(LSO23,10),SL(LHSO3,10),SL(LSO3,10)
C     * ,SL(LHSO,10),SL(LCO2,10)

C      PRINT*, ''
C      PRINT 195
C      PRINT 184, DIZ(LHQO,1),DIZ(LHQONO2,1)
C      PRINT 184, DIZ(LHQO,10),DIZ(LHQONO2,10)
      PRINT 183
      PRINT 184, DIZ(LHO2,1),DIZ(LHO2NO2,1)
      PRINT 184, DIZ(LHO2,10),DIZ(LHO2NO2,10)
C 
...
!      ENDDO
 ...
 235  FORMAT(A6,1x,1P10E12.4)
      ...
      print*,''
      print 197,'Alt(km)',(ISPECI(I),I=30,31)
      DO J=1,3
      print 198,j-0.5,(USOLI(I,J),I=30,31)
      END DO      
      DO J=4,NZ,2
      print 198,j-0.5,(USOLI(I,J),I=30,31)
      ENDDO
      print 197,'Alt(km)',(ISPECI(I),I=30,31)
      print *,''
      print 235, 'TPI',(TPI(K),K=30,31)
      print 235, 'TLI',(TLI(K),K=30,31)
...
C  
      ...
      print 197,'Alt(km)',(ISPECI(I),I=32,36)    
      DO J=1,NZ,4
      print 198,j-0.5,(DIZ(I,J),I=32,36) 
      ENDDO                                    
      print 197,'Alt(km)',(ISPECI(I),I=32,36)
  209 format(A48)
